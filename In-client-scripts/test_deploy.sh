...
add_user() {
	echo -e ""
	echo -e ""
	echo -e "-------- Adding user 'test'"
	echo -e ""
		
	if id "test" >/dev/null 2>&1; then
		echo "test:$1" | chpasswd
		echo -e ">> User 'test' already present, passphrase changed"
	else
		useradd -m -p $1 test
		[ $? -eq 0 ] && echo -e ">> User 'test' has been added to system encrypted" || echo -e ">> Failed to add 'test' user"
	fi
}
...
generate_conf() {
	...
	echo -e ">> /home/test/.vtorrent/vtorrent.conf"
	echo -e ""	
	echo -e "server=1"
	echo -e "daemon=1"
